...
export class UsageStatisticsClient implements UsageStatisticsApi {
  ...
  public async getTemplateStatsByName(
    templateName: string,
  ): Promise<TaskRun[]> {
    const encodedName = encodeURIComponent(templateName);
    const baseUrl = await this.discoveryApi.getBaseUrl('usage-statistics');
    const url = new URL(
      `usage-statistics/template/by-name/${encodedName}`,
      baseUrl,
    );
  ...
  public async getMonthlyStatsByTemplateName(
    templateName: string,
  ): Promise<MonthlyStat[]> {
    const encodedName = encodeURIComponent(templateName);
    const baseUrl = await this.discoveryApi.getBaseUrl('usage-statistics');
    const url = new URL(
      `usage-statistics/template/by-name/${encodedName}/monthly`,
      baseUrl,
    );
